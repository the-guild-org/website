...
export const CODEGEN_LOGO = {
  html:
    ...
    separator +
    `<a class="hive-focus -my-1 flex items-center gap-1.5 rounded-md p-0.5" href="${basePath}" aria-label="GraphQL Codegen home">` +
    `<img alt="" aria-hidden="true" class="h-6 w-auto max-sm:h-5" src="${basePath}/assets/img/gql-codegen-icon.svg" />` +
    `<span class="text-green-1000 dark:text-neutral-200 text-lg font-medium tracking-tight max-sm:hidden">Codegen</span>` +
    `</a>` +
    `</span>`,
};
