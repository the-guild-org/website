...
import { GraphFlow } from "#mdx-shims/graph";
...
- Hive CLI → _publish app deployment_ → Hive Registry / CDN
...
<GraphFlow title="Persisted documents">

- App → _sends a persisted document request_ → GraphQL API or gateway
- GraphQL API or gateway → _loads persisted documents_ → Hive CDN
- GraphQL API or gateway → _reports persisted document usage_ → Hive CDN

</GraphFlow>

The GraphQL API or gateway can be Hive Gateway, GraphQL Yoga, Apollo Server, or Apollo Router.
