...
import { GraphCompare } from "#mdx-shims/graph";
...
For CI/CD pipelines, we recommend checking schema changes in pull requests, pushing an immutable
schema revision during the merge or release flow, and publishing that revision when the service is
deployed. See the [schema revision workflow](/docs/other-integrations/ci-cd#schema-revision-workflow)
for a complete example.

Every schema published to Hive consists of the following data (depends on project type):

<GraphCompare title="Published data">
...
By default, Hive records an invalid schema version when a Federation publication fails supergraph or
contract composition. The Hive CDN continues serving the latest valid supergraph. Pass
`--fail-on-composition-error` to [`hive schema:publish`](/docs/api-reference/cli#fail-on-federation-composition-errors)
to reject the publication without creating the invalid schema version.
