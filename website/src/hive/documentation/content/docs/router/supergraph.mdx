...
The first argument is the supergraph's name. The configured supergraph is named `default`, and
plugin-owned variants use the name you give them. The router reports it as `supergraph_name` in the
[request summary log](/docs/router/observability/logging#access-logs), as `hive.supergraph.name` on
the `graphql.operation` span, and as the `supergraph.name` label on
[`http.server.request.duration` and `hive.router.graphql.errors_total`](/docs/router/observability/metrics),
so you can break down latency and errors per variant.
...
let supergraph = Arc::new(Supergraph::from_sdl("basic", sdl, options)?);
```
