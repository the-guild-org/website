...
import { GraphTimeline } from "#mdx-shims/graph";
...
- Phase 1: Measurement — both modes on `measure`, collect cost histograms
- Phase 2: Baseline setting — `max` at the 99th percentile, switch to `enforce`
- Phase 3: Gradual tightening — lower `max` as clients optimize their queries
- Phase 4: Enforcement with telemetry — alerts on rejections, keep tuning the cost model
