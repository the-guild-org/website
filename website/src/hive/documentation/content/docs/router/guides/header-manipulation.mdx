...
1. Any `no-store` or `no-cache` directive in any subgraph short-circuits the result to
   `no-store, no-cache`. The most restrictive directive always wins. Qualified forms such as
   `no-cache="set-cookie"` are treated as the unqualified, more restrictive directive.
2. `private` is kept if **any** subgraph sets it, and it overrides `public`. It does not escalate to
   `no-store`: the other directives are still merged, so a result like `private, max-age=50` stays
   cacheable by private caches (the browser) while shared caches (CDNs, proxies) won't store it.
   `private="field"` is treated as plain `private`.
3. Durations use the **minimum** of the values that are present. Subgraphs without the directive
   are ignored:
   - `max-age`
   - `s-maxage` - kept only when every merged `Cache-Control` value sets it. Because shared caches prefer `s-maxage`
     over `max-age`, the merge caps the result at the shortest effective shared-cache lifetime, so
     mixing the two can never extend how long a CDN keeps the response. For example, `max-age=100,
     s-maxage=50` merged with `max-age=80` gives `max-age=50`.
   - `stale-while-revalidate`
   - `stale-if-error`
4. Grants hold only when **every** subgraph sets them:
   - `public` - if a single subgraph omits `public`, is `private`, or does not emit a
     `Cache-Control` header at all, the merged value does **not** contain `public`.
   - `immutable` - same rule as `public`.
5. Restrictions are kept if **any** subgraph sets them: `must-revalidate`, `proxy-revalidate`,
   `must-understand`, and `no-transform`.
6. An unknown directive, or a duration without a value or with a non-numeric value
   (`max-age=abc`), results in `no-store, no-cache`. The router can't tell whether an unknown
   directive was meant to be restrictive, so it treats it as if it was, instead of ignoring that
   subgraph's value. Flag directives keep working if they carry an argument: `public=1` is merged as
   `public`, and the argument is ignored.

For example, merging `public, max-age=300, stale-while-revalidate=60` with
`private, max-age=120, no-transform` produces `private, max-age=120, stale-while-revalidate=60,
no-transform`.
