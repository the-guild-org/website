...
6. A value the router can't fully understand results in `no-store, no-cache`: an unknown
   directive, a duration without a value or with a non-numeric value (`max-age=abc`), or any other
   malformed directive. The router can't tell whether an unknown directive was meant to be
   restrictive, so it treats it as if it was, instead of ignoring that subgraph's value.
