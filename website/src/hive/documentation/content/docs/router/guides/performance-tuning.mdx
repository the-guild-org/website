...
<GraphFlow title="Circuit breaker">

- **Closed** → _rolling error rate over the last `volume_threshold` calls ≥ `error_threshold`_ →
  Open
- **Open** → _`reset_timeout` elapsed_ → Half-open
- **Half-open** → _probe error rate over the last `half_open_attempts` probes < `error_threshold`_ →
  Closed
- **Half-open** → _probe error rate over the last `half_open_attempts` probes ≥ `error_threshold`_ →
  Open
