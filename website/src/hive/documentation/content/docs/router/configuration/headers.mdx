...
- Durations (`max-age`, `s-maxage`, `stale-while-revalidate`, `stale-if-error`) use the shortest
  value. Unknown directives and missing or non-numeric duration values result in
  `no-store, no-cache`. Arguments on flag directives such as `public=1` are ignored.
