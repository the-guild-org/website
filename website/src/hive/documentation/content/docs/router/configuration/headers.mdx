...
- `no-store` or `no-cache` from any subgraph produces `no-store, no-cache`.
- `private` from any subgraph overrides `public` but remains `private`, preserving directives such as
  the minimum `max-age` for private caches.
- `max-age`, `s-maxage`, `stale-while-revalidate`, and `stale-if-error` use their minimum effective
  values. The merge accounts for shared caches preferring `s-maxage` over `max-age`.
- `public` and `immutable` require every subgraph to set them, while revalidation and transformation
  restrictions are preserved if any subgraph sets them.
- Malformed or unmodeled directives produce `no-store, no-cache` rather than being ignored.
- Mutations and errored responses produce `no-store, no-cache, must-revalidate`.
