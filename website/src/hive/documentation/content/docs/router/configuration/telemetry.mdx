...
| `process_variables`    | `boolean`                            | `false`                              | Report only the input fields present in each request's variables. See [Processing variables](/docs/router/observability/usage_reporting#processing-variables). |
...
| `parent_based_sampler`     | `boolean` | `false` | Inherit sampling decisions from the parent span. With Datadog, `false` defers an incoming sampled decision to Datadog's own sampling policy. |
...
`parent_based_sampler: true` makes Datadog honor a sampled remote parent. With `false`, the router
defers the inherited decision so Datadog applies its configured sampling policy, while trace and
parent IDs still propagate. Mixed OTLP, stdout, and Hive processors receive
