...
<GraphFlow title="Stages">

- Client request → **`router.request`** → Read GraphQL request → Parse and validate GraphQL →
  **`graphql.request`** → auth, JWT, policy, progressive labels → **`graphql.analysis`** → Plan
  GraphQL operation → Execute query plan → **`graphql.response`** → Build HTTP response →
  **`router.response`** → Client response
- Each stage → _HTTP request_ → Coprocessor service → _HTTP response_ → back to the same stage
