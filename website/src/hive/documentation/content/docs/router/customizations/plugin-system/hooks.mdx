...
its schema first. Both constructors take a name as their first argument. The router reports this
name in [logs](/docs/router/observability/logging#access-logs), [traces](/docs/router/observability/tracing),
and [metrics](/docs/router/observability/metrics) to show which supergraph served a request. The
supergraph loaded from `supergraph.source` is always named `default`.

Both constructors also take a complete `SupergraphOptions` value: planner options,
...
This hook gives plugins read-only access to the Router's query planning process. Plugins can
inspect the operation being planned and the resulting plan, and short-circuit the request with an
error, but they cannot replace the plan the Router executes. To change what gets planned, filter
the operation in [`on_graphql_analysis`](#on_graphql_analysis) instead.
...
operation parsing and validation. `end_payload.query_plan()` returns a shared reference, and
`end_payload.cache_hint` tells you whether the plan came from the query plan cache.
