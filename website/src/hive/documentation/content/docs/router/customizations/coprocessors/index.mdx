...
- Client request → GraphQL parsing and validation → **`graphql.analysis`** → Inject active labels for
  Progressive Override → GraphQL execution → Client response
- **`graphql.analysis`** → _HTTP request_ → Coprocessor service → _HTTP response_ → Inject active
  labels for Progressive Override
