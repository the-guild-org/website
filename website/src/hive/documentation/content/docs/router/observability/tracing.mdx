...
A response can contain several errors with different codes, so the router does not set a single
`error.type` on the root or operation span. Each error is recorded as a `graphql.error` span event
