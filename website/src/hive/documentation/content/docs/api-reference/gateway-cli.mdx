...
which composes a supergraph from the subgraphs running on your machine.

- `DEV_REMOTE`: Compose through the Hive registry instead of locally
...
- `DEV_REGISTRY_TOKEN`: The registry access token used for remote composition
- `DEV_TARGET`: The target to compose against, as `organization/project/target` or a target UUID
