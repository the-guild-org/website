...
The revision must already exist for the target's project and, for a distributed schema, the specified
service. Hive publishes the exact SDL stored for that immutable revision.
...
[Local Subgraphs (Dev)](/docs/gateway/supergraph-proxy-source).
