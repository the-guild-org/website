...
<Callout type="info">
  ...
  configuration used for schema checks and publishes. Like `hive dev --remote`, it requires an
  access token with the **Compose schema** permission for the project and target, and is only
  available for Apollo Federation projects.
</Callout>
...
<Callout type="info">
  `--hive-target` and `--hive-access-token` are the gateway's global Hive registry options, not
  dev-specific ones. They must be set together, and they also enable
  [usage reporting](/docs/gateway/usage-reporting) to that target with the same token, so the token
  needs the usage reporting permission as well or its reports are rejected. Only
  `--hive-access-token` is used for composition; `--hive-usage-access-token` and
  `--hive-trace-access-token` are not. To compose remotely without reporting usage, set `token` and
  `target` in the configuration file and only pass `--dev-remote`.
</Callout>
...
until a later poll composes successfully. If the first composition after start fails, the gateway
exits, so start your subgraphs before the gateway.
