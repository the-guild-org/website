...
<GraphFlow title="Signing">

- Consumer → _GraphQL request_ → Hive Gateway → Execution engine
- Execution engine → _subgraph request_ → **AWS SigV4 plugin** → Assume role (IAM) → Sign the
  request → _HTTP request_ → Products subgraph
- Execution engine → _subgraph request_ → **AWS SigV4 plugin** → Hard-coded configuration → Sign the
  request → _HTTP request_ → Users subgraph
