...
or with Vite React, update your `vite.config.ts` to add the following (the `plugins` option needs
`@vitejs/plugin-react-swc` 3.1 or newer; with the plain `@vitejs/plugin-react` or an older version,
`react()` does not accept SWC plugins):
