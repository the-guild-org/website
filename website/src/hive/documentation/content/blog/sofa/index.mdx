...
That gave the API fully generated REST endpoints, Swagger live documentation and a GraphiQL editor,
all from the same GraphQL schema. (The SpaceX Land API has since been shut down, so those live links
are gone; the commit above still shows everything it took.)
