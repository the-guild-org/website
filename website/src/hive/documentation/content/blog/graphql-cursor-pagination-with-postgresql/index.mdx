...
```sql title="Offset-based Pagination"
SELECT
  "id",
  "name"
FROM
...
```sql title="Cursor-based Pagination"
SELECT "id", "name"
FROM "users"
WHERE "id" > $lastId
ORDER BY "id" ASC
...
```diff title="Dataset users table"
| "id" | "name"   |
| ---- | -------- |
| 1    | 'Laurin' |
| 2    | 'Uri'    |
...
```diff title="Initial SQL query result"
| "id" | "name"   |
| ---- | -------- |
| 1    | 'Laurin' |
| 2    | 'Uri'    |
...
```sql title="Follow-up SQL query" {3}
SELECT
  "id",
  "name"
FROM
...
```diff title="Follow-up SQL query result"
| "id" | "name"   |
| ---- | -------- |
| 3    | 'Dotan'  |
| 4    | 'Dima'   |
...
```diff title="Dataset users with same created_at date"
| "id"                                   | "name"   | "created_at"                       |
| -------------------------------------- | -------- | -----------------------------------|
| '7c2730cb-a973-4a0a-9e9a-f8aa0044d321' | 'Dima'   | '2023-01-10T10:32:07.853915+00:00' |
| '628995bf-2907-49d1-a36f-978566a053c4' | 'Dotan'  | '2023-01-11T10:32:07.853915+00:00' |
...
```sql title="SQL query using compound cursor"
SELECT
  "id",
  "name",
  to_json("created_at") AS "created_at"
...
```sql title="SQL index for compound cursor"
...
```diff title="Dataset users with roles"
...
```sql title="SQL index for compound cursor with additional filter"
