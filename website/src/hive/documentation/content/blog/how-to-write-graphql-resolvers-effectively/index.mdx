...
```graphql title="src/graphql/schema.graphql"
type Query {
  movie(id: ID!): Movie
}
...
```ts title="src/graphql/schema.mappers.ts"
type MovieMapper = {
  id: string;
  movieName: string;
};
...
```ts title="src/graphql/resolvers/Query/movie.ts"
...
```ts title="src/graphql/resolvers/Movie.ts"
...
```ts title="src/graphql/resolvers/Actor.ts"
