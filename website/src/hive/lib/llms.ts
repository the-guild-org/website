...
export function getLlmsText(entries: DocsEntry[], navigation: DocsNavNode[]) {
  const entriesByPath = new Map(
    entries.map(entry => [`/docs${getDocsSlug(entry) ? `/${getDocsSlug(entry)}` : ''}`, entry]),
  );
  const link = (page: DocsNavPage) => {
    const data = entriesByPath.get(page.href)?.data as { description?: string } | undefined;
  ...
  return ['# Hive Platform', '', '> Hive Platform documentation', '', ...sections].join('\n');
}
