...
describe('getLlmsText', () => {
  test('generates an ordered index of absolute Markdown links', () => {
    ...
    const navigation = [
      ...
      {
        children: [{ href: '/docs/gateway', title: 'Getting Started', type: 'page' }],
        title: 'Hive Gateway',
        type: 'folder',
      },
    ...
    expect(output).toContain('- [Introduction](https://the-guild.dev/graphql/hive/docs.md)');
    expect(output).toContain(
      '- [Getting Started](https://the-guild.dev/graphql/hive/docs/gateway.md): Gateway documentation',
    );
    expect(output.indexOf('## Introduction')).toBeLessThan(output.indexOf('## Hive Gateway'));
  });
});
