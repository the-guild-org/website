---
title: Highly available CDN Access Tokens on Cloudflare Workers
tags: []
authors: laurin
date: 2023-01-30
description:
  Learn how we are using Cloudflare Workers to provide highly available access tokens for our CDN.
image: # /blog-assets/graphqxl-language/cover.png
thumbnail: # /blog-assets/graphqxl-language/thumbnail.png
---

As we are building GraphQL Hive we are constantly facing interesting problems and challenges that we
need to solve.
...
The avilability of the GraphQL schema artifacts is crucial for the GraphQL Hive clients to work
flawless even if there is an incident on the GraphQL Hive side.

The CDN is consumed by GraphQL Gateways and is critical for the success of our customers.

So far the GraphQL Hive CDN was using a simple token based authentication. The token was generated
based on the target id (think of it as project id) and an applpication secret and thus static.

That means if this key would accidentially leak to the outside world it would be impossible to
invalidate it.

Thus, we needed a new solution that allows generating and invalidating many different access keys in
parrallel, without drastically impacting the performance of the CDN.
...
First of all we needed to decide upon a store in which we can store the hashes of the CDN access
tokens.
...
Cloudflare Workers KV was not an option as GraphQL Hive supports both hosted (cloud) and self-hosted
(community) deployments, where the latter should be able to run without the need for ANY external
(paid) cloud services. We did not have a KV replacement available, on the other site there are many
S3 compatible storage solutions such as MinIO available.
...
some conceptulizing we came up with the following phases:

- Phase 1: Automatically write new "generated" legacy access tokens to the R2 buckets
- Phase 2: Run a migration for import all "legacy" access tokens in the R2 buckets
- Phase 3: Roll out the new CDN access token validation for legacy tokens
- Phase 4: Roll out the new CDN access token format and managment UI in the web application

## Phase I

At a first glance Phase 1 and Phase 2 almost seem identical, however the distinguish between these
...
This was the most scariest phase. After we seeded the new access token storage, it was now time to
switch the token validation method.

Instead of veryifing the CDN access token using an encryption secret environment variable we would
...
in that key there against the provided access key (the `X-Hive-CDN-Key` header value).

In case the key does not exist or the hash would not match the access token that means that the
token is no longer valid/invalid, which is later on our mechanism for allowing users to invalidate
the legacy token.
...
That means of a access token is invalidated (the R2 key is deleted), it might take up to 5 minutes
until the change is propagated. However, we felt like this is a great trade-off for by-passing
reading from R2 and running bcryppt for verifying the access token against the hash.
...
![Fire Fire Fire](/blog-assets/highly-available-cdn-access-tokens-on-cloudflare-workers/phase-3-rollout-errors.png)

We quickly responded with rolling back the latest changes and then it was time for debugging...

![Revert](/blog-assets/highly-available-cdn-access-tokens-on-cloudflare-workers/phase-3-revert.png)

## Phase 3.1

Now we needed to figure out what exactly was going wrong that let the new validation method fail.
This was especially cumbersome as debugging Cloudflare Worker code is fairly cumbersome.

We were already using [Cloudflare Analytics](https://www.cloudflare.com/analytics/) for exporting
metrics in out Grafana Dashboard. So the first idea was to add the new validation method with
...
After a bit of careful and manual thinkering with the production database and a deployment later,
the database migration from Phase 2 was now run with the correct secret.

On the dashboard we could now finally see the new validation method to succeed.

![Rollout](/blog-assets/highly-available-cdn-access-tokens-on-cloudflare-workers/phase-3-1-rollout.png)
...
![All good](/blog-assets/highly-available-cdn-access-tokens-on-cloudflare-workers/phase-3-1-all-good-in-da-hood.png)
...
The `"hv2"` prefix is a convinience for us for quickly identifying whether an access key sent to us
...
The UI part was pretty straight-forward and less challanging to build, however it was still part of
this project.

The new token overview:

![All good](/blog-assets/highly-available-cdn-access-tokens-on-cloudflare-workers/phase-4-cdn-access-token-overview.png)

Creating a new token:
![All good](/blog-assets/highly-available-cdn-access-tokens-on-cloudflare-workers/phase-4-token-create-form.png)

We successfully deployed this to production and then informated all our clients that are waiting for
this feature. 🎉 In addition this is of course now also available for the self-hosted Hive users.
...
Nevertheless, we are hapy to finish this project successfully and are looking forward to all the
future challenges!
