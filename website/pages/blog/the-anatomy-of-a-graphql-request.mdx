...
You might want to restrict what kind of operations are allowed to be executed. E.g. If we only
want to allow `query` operations, we can provide a custom validation rule that yields errors as soon
as a `mutation` or `subscription` operation is encountered.
