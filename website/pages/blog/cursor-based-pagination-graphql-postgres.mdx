...
image: /blog-assets/<placeholder>/cover.png
thumbnail: /blog-assets/<placeholder>/thumbnail.png
---
...
This guid will cover PostgreSQL concepts for building and optimizing paginated SQL queries.
...
Compared to offset-based pagination, we need pick a column and need to use a value of that column in
order to fetch the next items.
...
One of the main benefist of cursor based pagination is better query consistency (no skipped or
...
Afterwards we use the cursor (last `id` in the result set) to fetch the next two items.
...
that is the `created_at` column that are often added to all tables within a database in order to
keep track of when a row was created.
...
<Callout type="info">
  ...
  pagination it is generally recommend to treat dates as string as parsing them to numbers and then
  back can lead to subtle bugs, especially in languages like JavaScript where numbers are a bit
  complicated.
</Callout>
...
However, this will yield us zero results this time.
...
<Callout type="info">
  When you introduce an filter to a SQL query it is always important to add it to the index in order
  to gurantee fast responses. A SQL operation that took 1 second to execute could suddenly start
...
**This is also why we do not recommend to allow arbitary filters to be applied to your pagination
through user-input, as you might not be able to gurantee the performance of your application.**
...
By now, you should be aware of the pitfalls and things to consider when implementing cursor
pagination in a PostgreSQL database. Namingly to use stable cursor values and keep setting indexes
for your paginated queries.
