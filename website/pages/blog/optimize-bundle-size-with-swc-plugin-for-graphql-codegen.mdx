---
title: Optimize Bundle Size with SWC Plugin for GraphQL Code Generator
authors: jessevelden
tags: [client-preset, codegen]
date: 2023-02-24
description:
  'Optimize your Bundle Size with the SWC plugin for client-preset for GraphQL Code Generator'
# image: /blog-assets/optimize-bundle-size-with-swc-plugin-for-graphql-codegen/coverTODO.png
# thumbnail: /blog-assets/optimize-bundle-size-with-swc-plugin-for-graphql-codegen/thumbnailTODO.png
---

With the new
[`client-preset` for GraphQL Code Generator](https://graphql-code-generator.com/plugins/presets/client-preset)
...
const GetCartByIdDocument = graphql(/** GraphQL */ ` query GetCartById($id: ID!) {
   id
   items { 
      id
      ...CartItem
...
The generated code output for the `graphql` function is a function that looks similar to
the following code, where code-splitting and tree-shaking is not easily possible.
...
export function graphql(source: "fragment CartItems on CartItem {\n name\n quantity\n }\n "): (typeof documents)["\n fragment CartItem on CartItem {\n name\n quantity\n }\n "];
...
This means that every time you use the `graphql` function, you are importing basically all used
operations in your project, even if those are not being used. This can be a problem if you have a
lot of queries and mutations in your project.

For a more information/ history of this functionality, you can read more about it in the former 
[`gql-tag-operations-preset`](https://the-guild.dev/graphql/codegen/plugins/presets/gql-tag-operations-preset) that 
is now replaced by the `client-preset`. This idea was originally explored by [Maël Nison](https://github.com/arcanis) in 
[this Github project](https://github.com/arcanis/graphql-typescript-integration).

## The Solution: A Compiler Plugin

In order to have type safety in This is where a compiler can make a big difference. Instead of using
the map which contains all GraphQL operations in the project, we can use the generated document types for that 
specific operation. So the example above would compile to:
