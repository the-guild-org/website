...
✅ All gateways tested were able to process all requests without any failures (side note: when CPU limit is lower, some JS gateways are failing to respond to requests, and the requests are failing with a timeout or connecting drop).
...
📈 The peak memory usage of **Apollo-Router** was around 400MB, while **Wundergraph** needed more resources (x3, 1.3GB) to handle the same amount of requests. Memory consumption of **GraphQL-Mesh** in the test above was 550MB on Bun, and 1.4GB on NodeJS 18. 
...
📈 The ability of **GraphQL-Mesh** to use NodeJS's `cluster` feature pays off when more CPUs are available to use. 
...
- **Runtime environment matters:** NodeJS is great, but we are keeping an eye on other runtimes, such as Deno and Bun. We
  are also looking forward to seeing more solutions built with these runtimes. Rust keeps RAM lower when written correctly, and Go is also a great option for building GraphQL gateways, but memory consumption could be higher.
