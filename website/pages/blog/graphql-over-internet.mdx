...
the server fulfil that request. Llike in real world - those that know the language can communicate.
Please keep in mind, GraphQL is not a database nor is it a server - it is simply - **just a
language**.

### GraphQL Is like a Pizza Order

Imagine ordering a pizza - you call and ask for a pizza with special toppings, then the person on
the other end takes the order and tells the pizza chef what to make. Well, GraphQL is kinda' like
that phone call, except that instead of ordering pizza, you ask a server for some data; and instead
of speaking to a person, you're sending a specially formatted message to a server called a GraphQL
...
**However, neither of you know, or care, about how a telephone works!**

Synonymously, GraphQL itself deliberately does not care about the transport - it doesnt care about
how you send the request, how you receive the response, what is the delivery path, all of this is
out of the GraphQL's scope. It is quite literally just a data language.
...
a group of superheroes formed in 2019 to solve exactly this problem. A work group of avid GraphQL
lovers all with the same goal - standardise the process of transporting GraphQL over HTTP. The group
...
There is already a
[specification being standardised named GraphQL over HTTP](https://graphql.github.io/graphql-over-http/).
...
[current official GraphQL over HTTP spec](https://graphql.github.io/graphql-over-http/). It does so
in the clearest and most explicit manner so that others implementing their own server can quite
...
maintains the list of compliant servers and their reports - so please do PR your own implementation,
...
Of course, security is very important - the website audits run exclusively in the browser, there is
no tracking, no data sharing and there are no external dependencies either! Meaning, you can
download the website on your computer and have a portable offline checker.
...
overview of the history. Originally there was this library
[`subscriptions-transport-ws`](https://github.com/apollographql/subscriptions-transport-ws) that was
built and maintained by [Apollo](https://www.apollographql.com/). It also had an
[accompanying protocol](https://github.com/apollographql/subscriptions-transport-ws/blob/master/PROTOCOL.md).
But as time passed, they've been able to spend less time on it, probably due to increase demand in
other areas. The repo grew in issues, PRs were never reviewed - it eventually became unmaintained
and therefore deprecated. But this library actually kickstarted and pushed the evolution, it was a
big inspiration and a great influence for the upcoming refresh!
...
Full-duplex is just a fancy word for bidirectional communication - meaning, both the server and the
client can send each other messages through the connection channel whenever they want. When used in
conjunction with GraphQL, it allows clients to subscribe and receive events from the server in
real-time. This makes it ideal for applications that require real-time updates and push
notifications.

The
[GraphQL over WebSocket protocol](https://github.com/graphql/graphql-over-http/blob/main/rfcs/GraphQLOverWebSocket.md)
leverages all pros of the WebSocket and gives shine to real-time GraphQL. Please note that it is
**not** backwards compatible with
[Apollo's protocol behind `subscriptions-transport-ws`](https://github.com/apollographql/subscriptions-transport-ws/blob/master/PROTOCOL.md).
...
very simple and barebone. But don't be mistaken, it's very versatile and extendable!

One key element people often struggle with WebSockets are reconnections and `graphql-ws` is all
about stable connections. Silent retries with custom strategies are easy to use and extend.
...
That's an excellent question actually! Here's a few gotchas' about the browser native `EventSource`:

- Headers cannot be customised
...
Keeping everything from the previous section in mind, the
[GraphQL over SSE spec](https://github.com/graphql/graphql-over-http/blob/main/rfcs/GraphQLOverSSE.md)
...
- ["distinct connections mode"](https://github.com/graphql/graphql-over-http/blob/main/rfcs/GraphQLOverSSE.md#distinct-connections-mode) -
  basically the regular way you'd use SSEs, each request is a connection on its own
- ["single connection mode"](https://github.com/graphql/graphql-over-http/blob/main/rfcs/GraphQLOverSSE.md#single-connection-mode) -
  a single SSE connection is maintained for streaming, but other accompanying requests dictate the
  behaviour
...
SSEs, so I'll just breifly touch the obvious.
...
Check out the video of the related presentation that happened at the
[GraphQL Zurich Meetup](https://www.meetup.com/graphql-zurich/) sponsored by
[SwissLife](https://www.swisslife.ch/).
