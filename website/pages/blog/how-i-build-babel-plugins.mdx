...
- The parser parses the code string into a data representational structure called AST (abstract
  syntax tree) using
  [`@babel/parser`](http://github.com/babel/babel/tree/master/packages/babel-parser).
- The AST is being manipulated by pre-defined plug-ins which
  use[`@babel/traverse`](http://github.com/babel/babel/tree/master/packages/babel-traverse).
- The AST is being transformed back into code using
  [`@babel/generator`](http://github.com/babel/babel/tree/master/packages/babel-generator).
...
`"bar"` in case we assign the `"foo"` value to `foo`. We can add a manipulation handler for any
node type that we want, it can be `AssignmentExpression`, `Identifier`, `Literal`, or even
...
- The `types` object is a Lodash-esque utility library for AST nodes. It contains methods for
  ...
  [`@babel/types`](http://github.com/babel/babel/tree/master/packages/babel-types), and further
  ...
  `types` can be found
  [here](http://github.com/jamiebuilds/babel-handbook/blob/master/translations/en/plugin-handbook.md#babel-types).
- Just like the `types` object, the `scope` object contains utilities which are related to the
  ...
  [here](http://github.com/jamiebuilds/babel-handbook/blob/master/translations/en/plugin-handbook.md#scope).
...
[astexplorer.net](http://astexplorer.net) and for API docs I recommend you to work with this
wonderful
[handbook](http://github.com/jamiebuilds/babel-handbook/blob/master/translations/en/plugin-handbook.md).
