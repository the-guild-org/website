...
is going to be called, where the current fiber and its first hook node in the hooks queue are going to be
stored in global variables. This way, any time we call a hook function (`useXXX()`) it would know in
which context to run.
