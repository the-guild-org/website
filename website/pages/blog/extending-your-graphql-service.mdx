...
date: 2024-11-18
authors: egoodwinx
tags: [graphql, graphql-tools, schema-stitching, federation]
...
Similar to Apollo Federation, schema stitching allows for directives-based approach of schema
stitching. This allows you to configure your merging at the service level versus the gateway level.
...
Apollo Federation lets you use directives to combine multiple APIs into a single federated graph.
...
Apollo Federation introduces the concept of Entities which are federated types that exist across
multiple subgraphs.
...
Apollo Federation allows each subgraph to manage part of the unified schema independently. The
central gateway composes these subgraphs into a unified graph, and relationships between services
are handled through federation directives like `@key`, `@provides`, and `@extends`. The gateway uses
...
The Guild’s products are compatible with both Apollo Federation and Schema Stitching.
...
Apollo Federation requires more over knowledge of GraphQL from all teams involved as they need to
work together to make the federated schema work as expected. Schema Stitching requires less
knowledge from subschema teams but more knowledge from the team owning the gateway as the stitching
logic can become complex as the number of services grow.
...
Apollo Federation takes a bit longer to set up as it requires multiple subgraphs, a gateway, and a
registry. As the services grow, Apollo Federation can speed up development for teams since each team
...
Apollo Federation is more suited for scalability and team collaboration. Customization is more
structured through federation directives.
