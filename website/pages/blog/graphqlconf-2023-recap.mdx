...
to have such a dedicated leader. GraphQL to this day, from working groups, board meetings, to the
first open GraphQL Conf! We are lucky to have such a leader to our community! It was truly inspiring
to see the faces of the people we had only known through outdated avatars on GitHub or Twitter, who
are the driving force behind the vibrant GraphQL community.
...
3. Innovation has paved the way for numerous superior open-source solutions across every layer of
   ...
   vendor lock-in; developers can select the best tool for each specific layer for developers -
   separate the concerns between gateway, subgraph and schema registry - no need for vendor lock-in,
   choose the best tool for each layer
...
patterns of specification-less Federation protocols with a fresh approach.
...
SOAP, gRPC, etc.). On the other hand, other Federation protocols requires a specific subgraph
structure. Additionally, it was interesting to learn that even Facebook, with its vast resources,
finds value in a monolithic approach for their GraphQL implementation.
