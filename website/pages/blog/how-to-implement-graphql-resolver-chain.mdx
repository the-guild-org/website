...
1. This is what a resolver map: 
   a. It contains an object with keys that match the schema's types e.g. `Query`, `Movie`, `Character`.
   b. Each key of an object is a resolver function that resolves a field of the corresponding type. e.g. `Query.movie` resolves a `Movie` type.
2. This is where the `Query.movie` resolver resolves the `Movie` type. Note: we are not using the database yet to keep it simple.
   a. The object returned here corresponds to the `Movie` type in the schema.
   b. The returned object has `id` field and `name` field that matches the schema's `Movie` type, and will be returned to the client with the current implementation.
