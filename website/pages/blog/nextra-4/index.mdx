...
description:
  App Router, Turbopack, new Rust search Pagefind, RSC i18n, server/client components, compiled by
  React Compiler, GitHub Alert Syntax, new _meta.global file
date: 2025-01-10
authors: [dimitri]
...
Nextra introduce new option `whiteListTagsStyling` which allows you to whitelist html elements which
will be replaced with components defined in [`mdx-components.js` file](#use-mdx-components). By
default, Nextra replaces only `<details>` and `<summary>`, but with this option you can replace any html
element.
...
{/* Will be replaced and styled too */}
<h1>World</h1>
```
