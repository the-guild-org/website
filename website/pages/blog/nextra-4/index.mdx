...
### `<Th>`, `<Tr>` and `<Td>` Components were Removed

`<Th>`, `<Table.Tr>` and `<Td>` components were removed and attached directly on `<Table>`
component. This was done to better UX since these components are expected to use together with
`<Table>`.
