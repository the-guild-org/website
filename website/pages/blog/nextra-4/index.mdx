...
[Pages Router](https://nextjs.org/docs/pages) has been discontinued.

### Two Ways to Render

#### `content` Directory Convention

Migrate your Pages Router site with minimal changes using this mode. Here's what to do:
...
<h5>Set `contentDirBasePath` option in `next.config` file (_optional_)</h5>
...
<h5 id="add-mdxpath-page-jsx-file">Add `[[...mdxPath]]/page.jsx` file</h5>
...
Above [`content` directory convention structure](#add-mdxpath-page-jsx-file), with this mode will
looks like:
...
#### New Single `_meta.global.{js,jsx,ts,tsx}` File

You can now define all your pages in a single `_meta` file, suffixed with `.global`. The API remains
the same as folder-specific `_meta` files, with 1 exception: folder items must include an `items`
field.
...
### New Option `whiteListTagsStyling`
