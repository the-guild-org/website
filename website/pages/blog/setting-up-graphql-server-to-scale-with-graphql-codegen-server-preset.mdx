...
Putting many teams in one codebase without clear structure and guidelines is a recipe for disaster. The first team in the codebase usually sets it up a structure that works for them. When a second team joins, they most likely just follows the structure that's already there. The same story happens for every team thereafter. After a few more rounds, development slows down. One may review the structure and finds themselves staring down a codebase that was structured by one team, for one team.

Teams want their members to be notified for changes that need reviewed, not on every Pull Request (PR). If you were ever been notified for a change that is unrelated to your team, chances are the codebase is not structured to support many teams working in it.
...
├── src/
│   ├── schema/
│   │   ├── TeamA/  # Team A notified (by CODEOWNERS) if anything changes in this folder
│   │   │   ├── datasources/
│   │   │   │   ├── UserDatasource.ts
│   │   │   │   ├── AuthDatasource.ts
│   │   │   ├── resolvers/
...
│   │   │   ├── userSchema.graphql
│   │   │   ├── authSchema.graphql
│   │   ├── TeamB/  # Team B notified (by CODEOWNERS) if anything changes in this folder
│   │   │   ├── datasources/
│   │   │   │   ├── BookDatasource.ts
│   │   │   ├── resolvers/
│   │   │   │   ├── bookResolvers.ts
...
On top of that, all datasources and resolvers must be put together into the GraphQL server. In this case, it is the `server.ts` file. Who owns this file and other maintenance of the server such as package updates, security patches, `codegen.yml` (we all use GraphQL Codegen right? 😉) etc.?

### Solution: Split into Modules

It is common to see teams change name or mission. Yet, the thing that does not change very often (and in some cases, never) is business domain. If we split our code based on business domain and assign teams accordingly, it becomes much more scalable.

For server maintenance, it is best to have a small group of dedicated maintainers. This could consist of one member from each team in the codebase and this group's membership can rotate. Alternatively, some companies have a dedicated team of maintainers. Having a dedicated group - let's call them the Maintainers - helps reduce noise and cognitive load for the rest of the teams, allowing them to focus on delivering feature.
...
Bad practices and conventions spread like fire, on a hot summer day, in the bush, in Australia. Trust me it is the worst! I used to be part of a Maintainers team. We had guidelines various topics, one of them is resolver naming convention. Once in a blue moon, one person incorrectly used pascal case instead of camel case. The following day, I woke up with more than half of the resolvers in pascal case. 😱

OK, I am exaggerating here but the experience was very traumatising.

Guidelines alone is only good if people follow it. Without explicit and automatic enforcement, things start to slip and bad practices spread sooner or later. 

To enforce guidelines, we need tools. Luckily, these days we have an extensive range of tools for best practices a GraphQL server:
...
For example, the guideline for one of the GraphQL servers I am working on is to use generated types from GraphQL Codegen. Yet, some people do not follow the guidelines, mostly because they are not aware of it. Luckily, other team members or the Maintainers catch these issues at PR review time. But it could have saved everyone time and effort if we had tools that could enforce this guideline.
...
For example, if a new resolver is added, they must be manually added to the resolver map. So, Maintainers are notified on every new resolver. It may look like this if you are using [GraphQL Yoga](https://the-guild.dev/graphql/yoga-server):
...
There is a way to mitigate this issue: each schema module exports an object of resolvers and they are merged with [mergeResolvers from @graphql-tools/merge](https://the-guild.dev/graphql/tools/docs/schema-merging#merging-resolvers). This means the Maintainers are notified on every new module instead of every resolver:
...
However, `mergeResolvers` has a runtime and it is simply merging plain JavaScript objects. Thus, there is a risk of someone accidentally overriding others' resolvers. The overriding issue is hard to find in large codebases with hundreds of resolvers.

<img alt="mergeResolvers can cause conflicts and pain" src="/blog-assets/setting-up-graphql-server-to-scale-with-graphql-codegen-server-preset/merge-resolvers.gif" style={{width: "80%", margin: "0 auto"}}/>

Another commonly used feature is [mappers](https://the-guild.dev/graphql/codegen/plugins/typescript/typescript-resolvers#mappers). This feature allows resolvers to return mapper objects instead of GraphQL output types.
...
The resolver files will not get overwritten when codegen is run again. However, there are some smarts built-in to ensure resolvers are correctly exported. For example, if we rename `User` resolver to `WrongUser` in `src/schema/user/resolvers/User.ts`, then run codegen, the file will be updated with a warning:
...
*   Scalars:
    *   If the Scalar name matches one in [graphql-scalars](https://the-guild.dev/graphql/scalars), it is automatically imported from `graphql-scalars` into the resolver map so make sure to install it:
    ```bash
