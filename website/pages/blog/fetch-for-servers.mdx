...
[we launched GraphQL Yoga 2.0](https://the-guild.dev/blog/announcing-graphql-yoga-v2) - a server
...
contains all the details (headers, method, etc.) and the data stream you need for that communication.
Then you take the [`Response`](https://developer.mozilla.org/en-US/docs/Web/API/Response) object
that contains the connection stream, and process it as you want with `.json()`, `.arrayBuffer()`,
`.formData()` or just access the stream itself by `.body()` as `ReadableStream`
