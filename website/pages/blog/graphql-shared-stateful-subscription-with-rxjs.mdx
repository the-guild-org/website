...
- a `subscribe` function which is run one time only and must returns an AsyncIterable
...
In order to execute logic, we have 2 places: in `subscribe` when the subscription starts and in `resolve` on each emitted value. What if we want to execute logic when the subscription ends (whether it completed or was stopped by the client)?
