...
export const meta = {
  title: 'Announcing GraphQL Armor support for GraphQL Yoga 2',
  author: 'laurin',
  tags: ['graphql', 'graphql-tools', 'security'],
  date: '2022-08-24',
  description:
    'Bringing security by default to GraphQL Yoga',
  image:
...
A few weeks ago, the GraphQL Security company [Escape](https://escape.tech) released GraphQL Armor, an open-source middleware to add a security layer on top of GraphQL endpoints and mit´igate common attacks.

GraphQL Armor blocks abusive requests by putting reasonable and clever limits to Queries. To us, this represent the go-to solution when using [persisted operations](https://www.envelop.dev/plugins/use-persisted-operations) , like when building a public GraphQL API. Even if you are building an internal API these tools can be handy for preventing too heavy GraphQL queries. This technical approach is also complementary to the existing set of plugins for hardening endpoints: 
...
Thus we decided to work together with Escape's team towards adding the best support for Yoga 3 to GraphQL Armor.
...
Escape's team is actively working on improving Armor and its support for GraphQL Yoga
