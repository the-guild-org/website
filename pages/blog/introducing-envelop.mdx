...
export const meta = {
  title: 'Introducing Envelop!',
  author: 'dotan',
  tags: ['graphql', 'server', 'node', 'plugins', 'envelop'],
  date: '2021-07-20',
  description:
    'A new tool by The Guild, for making it easier to write GraphQL servers and custom execution flows.',
...
- Make “hard” **GraphQL capabilities easy** by installing **powerful plugins** (Caching, Tracing with Prometheus/DataDog/newRelic/Sentry/OpenTelemetry/ApolloTracing, Loggers, GraphQL-Jit, Persisted Queries, Security with rate-limit/depth-limit/Auth0 and many others from the [Plugins Hub](https://www.envelop.dev/plugins))
- **Solve once and share across the ecosystem** - Each plugin works with any HTTP server (Express, Fastify, /AWS Lambda/Azure Functions/Cloudflare Workers/Google Cloud Functions) and any schema builder (SDL, Apollo Federation, Nexus, TypeGraphQL and others)
- **Framework for Frameworks** - Envelop will become the new basis for GraphQL Frameworks. [It's already availble if you are using RedwoodJS](https://community.redwoodjs.com/t/migrate-your-redwood-graphql-api-with-graphql-helix-and-envelop/2264), and we have PRs open for Loopback, NestJS, Parse and others. Also, Envelop works across all of Apollo’s solutions.
...
- You need a HTTP server - it doesn’t matter what type of HTTP server.
...
[Here’s a list of plugins available](https://github.com/dotansimha/envelop#available-plugins) in Envelop right now (and the list grows every day!).
