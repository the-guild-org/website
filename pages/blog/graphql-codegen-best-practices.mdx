...
export const meta = {
  ...
  author: 'dotan',
  tags: ['graphql', 'codegen', 'typescript', 'react', 'angular', 'apollo'],
  date: '2020-05-20',
  description:
...
In this article, we'll cover the development workflow of frontend applications with _TypeScript_ and _GraphQL Code Generator_, suggest best-practices for GraphQL development for frontend developers, and try to explain the idea behind it.
...
<!-- GraphQL schema can be defined in many ways, but the output is the same - it's an HTTP endpoint, usually `/graphql`, which you can communicate with by sending `POST` request, with request body that contains a `query` (of type `string`) and `variables` (of type `object`).  -->
...
<!-- [TODO: gif of "any" in the component code] -->

With GraphQL Code Generator, you can have a full type safety, based on your GraphQL schema _and_ your GraphQL operations, and that means:
...
<!-- [TODO: gif of fully typed api in component code] -->
...
You can start by [trying GraphQL Code Generator plugin in the live-demo here](https://graphql-code-generator.com/) and with the [Getting started with GraphQL Code Generator](https://graphql-code-generator.com/docs/getting-started/installation).

<!-- ## Working Examples

We created a 2 repositories with a working application, that fetches data from remote service using GraphQL, Apollo and Angular/React.

You can run it and play with it here:

### React (with `react-apollo` and React Hooks)

[TODO: live demo here]

### Angular (with `apollo-angular` and RxJS)
...
<details><summary>Dump remote schema to a local file</summary>
...
<details><summary>Save local GraphQL Introspection</summary>

GraphQL schema can be represented in many ways. One of the is [introspection](https://graphql.org/learn/introspection/).
...
<details><summary>Add custom content to output files</summary>
...
<details><summary>React & Apollo: Generate Hooks</summary>
...
<details><summary>React & Apollo: Generate HOC (High-Order-Component)</summary>
...
<details><summary>React & Apollo: Generate Components</summary>
...
<details><summary>Angular & Apollo: Generate Services</summary>

You can generate ready-to-use Angular `Service`s for your GraphQL operations, with the following configuration:
...
<details><summary>React & Urql: Generate Hooks</summary>

If you are using [`urql`](https://github.com/FormidableLabs/urql) as your GraphQL client, you can generate ready-to-use React hooks for your GraphQL operations, with the following configuration:
...
<details><summary>Vue.js & Apollo: Generate Composition Functions</summary>

If you are using [`Vue.js`](https://vuejs.org) with `@vue/apollo-composable` your GraphQL client, you can generate composition functions based on your GraphQL operations:
...
<details><summary>Apollo: type-safe `refetchQueries`</summary>

If you are using Apollo Client, and you wish to refetch a query when a mutation is done, you can do add [`@graphql-codegen/named-operations-object`](https://graphql-code-generator.com/docs/plugins/named-operations-object) plugin to your setup.
...
<details><summary>Apollo: auto-generated `fragmentMatcher` / `possibleTypes`</summary>
...
This is needed in order to improve performance of Apollo store. [You can read more about this here](https://www.apollographql.com/docs/react/data/fragments/#fragments-on-unions-and-interfaces).
...
:::note
Please have some judgment before creating fragments, it should represent data structure that is specific per component.
Don't abuse this mechanism by creating fragments with a single field. Try to group it in a way that matches your components needs.
:::
...
The `@graphql-codegen/typescript-operations` is the TypeScript representation of your GraphQL selection set.
Just like selection set chooses fields from the GraphQL schema, `typescript-operations` picks fields from `typescript` plugin (which is the representation of your GraphQL schema).
