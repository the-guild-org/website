...
export const meta = {
  title: 'Orchestrating the Open Source Contribution Workflow',
  author: 'tvvignesh',
  tags: ['open source', 'contributor', 'workflow'],
  date: '2021-04-20',
  description: 'Sharing our journey in making open source contributions easier',
...
But as anything goes, it is not without its own set of challenges which advocates us to think of interesting solutions. This stands true in our case as well.
...
Now, [Urigo](https://twitter.com/UriGoldshtein) had an interesting thought process - since we had already published the contributor workflow guide, why not we label issues keeping the same in mind? All we needed to do was push an issue towards progress from Step 1 to Step N
...
which is how almost workflows look like for every project (Guild or even otherwise) - This is again the initial iteration and we will have more room for improvement over time and you may see more/lesser stages over time.

The next thing which we will be doing is labelling all the issues in all the repositories we manage appropriately against these stages which can give a better sense to both the maintainers and the contributors avoiding any ambiguity in the process.
