...
The next step is to implement the template for for server side and client side, the template engine I picked is [Handlebars.js](http://handlebarsjs.com/) because it comes with a set of great template utils (such as each, if and unless), but allows you to create a custom helpers — and this is a feature I wanted to preserve and allow other developers use when creating custom templates — so each language template can implement it own template, config and template helpers!
