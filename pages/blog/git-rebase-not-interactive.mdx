...
tags: [git]
date: 2018-09-03
description: 'How to build a Node.js script to re-write history. Pre-requisites: Familiarity with git rebase --interactive.'
...
_tl;dr: How to build a Node.JS script to re-write history. pre-requisites: Familiarity with `git rebase --interactive`._
...
BUT! Why does the `GIT_SEQUENCE_EDITOR` environment variable has to reference an actual text editor? What if we set that to reference Node.js' executable? Aha! JACKPOT!
...
gitRebaseInteractive(
  ...
  (operations, amount) => {
    ...
    const dropOperations = operations.slice(0, amount).map(operation => operation.replace('pick', 'drop'))
