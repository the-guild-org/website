...
If you're creating a GraphQL server you'd probably want to have a database behind it with somthing like Prisma. But GraphQL has some amazing tools that can make your life easier like the GraphQL code generator
which helps you create types for your resolvers.
...
For example, here is an example for a resolver without the codegen, as you can see, we need to give everything a type.
...
But, with the codegen, the types are no longer needed, because it genereates the types, so typescript will now know which types everything uses.
...
-Another problem is that sometimes, some prisma operations get arguments which are for filtering and paginating, now, if you have this type of filter in your GraphQL schema, it might look something like this:
