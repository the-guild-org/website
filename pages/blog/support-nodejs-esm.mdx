...
If you have been in the Node.js Ecosystem for some time and developing libraries, you have probably encountered the fact that ESM compatibility has been a struggle, behind experimental flags and/or broken for practical usage.
...
For the reason just mentioned, the standard "package.exports" should like something like this (even for single entry-point libraries, it won't hurt in any way):
...
#### What about \_\_dirname, require.resolve, require.cache?
...
There is a relatively small but effective way of automated testing for all the top-level imports in ESM, you can have an ESM script that imports every `.mjs` file of your project, it will quickly scan, importing everything, and if nothing breaks, you are good to go 👍, here is a small example of a script that does this, and it's currently used in projects that support ESM https://gist.github.com/PabloSzx/6f9a34a677e27d2ee3e4826d02490083, and you only need will need to add it in your CI.

## TypeScript

In regards to TypeScript supporting ESM, it divides into two subjects:
...
The original usage for this TypeScript feature [was not for this purpose](https://www.typescriptlang.org/docs/handbook/declaration-files/publishing.html#version-selection-with-typesversions), but it works, and it's a fine workaround until TypeScript actually supports it
...
> In [The Guild](https://the-guild.dev/) we use this method using tooling that creates the temporary package.json automatically. See [bob-the-bundler](https://github.com/kamilkisiela/bob) & [bob-esbuild](https://github.com/PabloSzx/bob-esbuild)
