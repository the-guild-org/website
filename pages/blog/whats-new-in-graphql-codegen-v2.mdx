...
## Why `v2`?
Codegen is constantly evolving, without the need for major and breaking changes, but there were a couple of things we wanted to do to make Codegen better and easier for you that require some small breaking changes. 
...
Based on your feedback we might make that the recommend way of using Codegen on the frontend in the future
...
## What's next?
Like we said at the beginning, we continuously keep improving Codegen, so this release is not the end but just a start.  Here are something things we are currently working on:
