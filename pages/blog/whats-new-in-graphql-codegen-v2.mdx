...
But other tools, which we use as a dependency, like `graphql-tools` already support ESM (v8), and dropped support for Node 10, so we are aliging to that.
