...
Step 1: npm install the [`sofa-api`](https://npmjs.com/package/sofa-api) package and add the following line of code:
...
Right now Sofa has a built-in support for [Express](https://expressjs.com) but it's totally possible to use a different framework. The main concept stays exactly the same so only the way we handle the request differs across different server implementations.
...
Thanks to [Kamil Kisiela](https://twitter.com/kamilkisiela) for working with me on this and making this library a reality!
