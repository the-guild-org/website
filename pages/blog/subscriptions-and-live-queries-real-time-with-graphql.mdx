...
But you are not forced to use WebSockets at all. HTTP Long-Polling (e.g. via [Server Side Events](https://developer.mozilla.org/en-US/docs/Web/API/Server-sent_events/Using_server-sent_events)) might be a more lightweigt solution. For both your server and your client.
...
Convince yourself by comparing the client code of [`graphql-ws`](https://github.com/enisdenjo/graphql-ws/blob/master/src/client.ts) to TODO INSERT GRAPHQL HELIX TRANSPORT CODE.
...
With this configuration relay can now execute subscription operations. Because the graphql-ws protocol is way more complex than the GraphQL over HTTP protocol, we use the client exported from teh graphql-ws package instead. Some additional bundle-size we got there. As mentioned before SSE might be a better leight-weight alternative.

That aside, lets start with a basic subscriptions that should update one of our components.
...
It is not limited on notifying whether the `totalLikeCount` of the post likes have changed, in the future we could adjust the selection set on the post field as it also returns a `Post` type similar to our already existing `Query.post` field.
...
This subscription is specifically designed for only a update of the `totalCount`. However, the subscription result returns no `Post` type. Therefore we cannot make use of the automatic cache updates via the `id`.
...
For a list containing hundreds of items only sending the changed item to the client might be the smarter solution...
...
Relay contributers have [worked on some nice query directives](https://github.com/facebook/relay/issues/3123) that allow reducing such cache update code. However, for a complex app you will eventually not be able to completly avoid it.

In all of the above examples we responded (more or less implicit) to data change events. **Responding to events.**
...
In my opinion that is exactly what subscriptions are meant for. Subscriptions are best suited for reacting to side-effects that don't affect the data in the cache.
...
Lets take a look at all those options!
...
The resolver implementation above only responds to emitted post changes. In order keep track of all the resources defined in a operation documents selection set, we will have to come up with a smart abstraction. We will take a look at such a abstraction later!
...
Okay, now we can select everything we could have selected with our query operation! Let's further delay the implementation part for one small topic we scratched before.

Imagine we already implemented the smart invalidation in the `subscribe` function of our `live` resolver. A resource got invalidated and the new result is derived from calling all those resolvers. We have a potentially huge payload and we send all of that back to the client...

Another, flaw existing with the implementations of Hasure and Postgraphile.

Ideally, we only wanna send a patch to the client that gives instructions how to get from the previous execution result to the next.

The RFCs and implementations for the `@defer` and `@stream` introduced ways of sending (delayed) partial results to clients. However, those "patch" operations are currently highly limited to a replace object and append to list operation.
...
The server implementation uses a event emitter and a immutable state tree for detecting changes that must be sent to clients. The patch is automatically generated from the next state emitted via the event emitter.

While the idea is quite nice the implementation is obviously meant for immutable data structures. Having to rewrite our existing GraphQL layer for supporting live queries is a big trade-off. Furthermore, the library is not maintained that well. My PRs for making the library compatible with recent GraphQL versions were not merged. Using unions and interfaces was impossible. Having to patch a library with patchc-package before even being usable is generally a bad sign.

So after having built an example app with `graphql-live-subscriptsions` from scratch, studing the flaws of that library and being uncomfortable with the vendor lock-in of services such as Postgraphile and Hasura, I decided to approach the problem of live queries in a more pluggable way.

---

Where full blown solutions give you a full solution that is less flexible I thought instead of solving everything in one library it might be better to seperate the issues into smaller issues.

1. Hava a common definition and set of utilities for determining of a live query

2. How can we keep track and invalidate resources selected by a live query in a efficient, but still generic way?

3. How can we efficiently send query updates to the clients?
