...
But as your business requirements might get more complex you might need to do manual cache updates.
...
As your application grows the manual cache update code can become so complex and confusing that I have considered switching back into simply refetching the queries in some application.
...
Before we take a look at how all of those implementations, let's break down what I would expect from a live query implementation.
...
1. Automatically updates execution result. **No.** ❌
...
1. Automatically updates execution result. **Yes.** ✅

When using services as Postgraphile and Hasura that is the case. However, if you have any additional resolvers that are added on top of the service schema you cannot implement invalidation for those. In user-land you will have to come up with your implementation of resource tracking by yourself. But don't be scared, continue reading, we will explore possible options later!

2. Efficiently send updates to client. **No.** ❌

The whole execution result is sent over to the client for every single time a live query got invalidated.

3. Don't rely on any specific database/vendor lock-in. **Kind of.** ✳️

For Hasure and Postgraphile? Yes. For your own solution? No!

4. Easy to adopt with existing GraphQL schema. **No.** ❌
...
If you implement your schema conform, this library delivers precise results once the immutable state has changed.

2. Efficiently send updates to client. **Yes.** ✅
...
4. Easy to adopt with existing GraphQL schema. **It depends.** ✳️

Adding a immutable layer to your existing schema is a big change.
...
For ensuring that the store keeps track of all your query resources you should always select the id field on your object types. The store will only keep track of fields with the name id and the type `ID!` (`GraphQLNonNull(GraphQLID)`).
...
If you are using a ORM library you can simply add a middleware for automatically invalidating resource. Here an example with prisma:
...
Regarding the transport. Both `graphql-ws` (GraphQL over WebSocket), `graphql-helix` (GraphQL over SEE) and `@n1ru4l/socket-io-graphql-server` (GraphQL over Socket.io), support this library out of the box.

[You can find all libraries and a example todo app that sync across all connected clients over here.](https://github.com/n1ru4l/graphql-live-query)
...
The execution result published by the AsyncIterator is always a complete execution result. But, a library that can be plugged and create JSON patches from the last to the next execution result is already prototyped and planned to be released soon!
This approach might be seen as a overhead but is IMHO the easiest pragmatic solution for the time being.
...
You can add this library to your existing GraphQL schema without hazzle. Add invalidation code to your mutation resolvers or as a ORM middleware and you are good to go!
