...
export const meta = {
  ...
  image:
    '/blog-assets/subscriptions-and-live-queries-real-time-with-graphql/cover.png',
};
...
The first major and wide-adopted transport implementation was (and probably is) [`subscriptions-transport-ws`](https://www.npmjs.com/package/subscriptions-transport-ws). It was developed by Apollo, but unfortunately they seem to have abandoned it since then. Fortunately, we now have a successor [`graphql-ws`](https://github.com/enisdenjo/graphql-ws).
...
const emitter = createPubSub(new EventEmitter());
```
...
By choosing the name `graphql-subscriptions` it gives the impression that the logic is specific to GraphQL and reduces other possible contributions form people that need a similar event abstraction.
...
The `query` field, which selects the selection set from `Query` tyoe and a `patch` field which is a JSON Patch operation.
...
The clients must then implement the logic for [applying the patch operation to their client cache](https://github.com/n1ru4l/graphql-live-chat/blob/master/packages/frontend/src/jsonpatch.ts). [Example app with both backend and frontend](https://github.com/n1ru4l/graphql-live-chat)
...
GraphQL Queries can become quite huge. Sending big payloads over the wire can be exepnsive at some point. JSON patch is a handy standard for only sending change instructions over the wire which can potentially reduce such huge query results.
...
- Better list diffing/format for live query patches (as JSON patch performs rather poorly on list diffs)
