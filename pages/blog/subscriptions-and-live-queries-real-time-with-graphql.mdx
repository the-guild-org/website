...
Subscriptions are the goto solution for adding real-time capabilities to a GraphQL powered application.
At the same time the term GraphQL Live Query floats around and can often be found the context of subscriptions.
...
Since promises can not represent multiple values over time, the `graphql-js` reference implementations uses [`AsyncIterator`](https://tc39.es/ecma262/#sec-asynciterable-interface), which is a native structure similar to Obserables (which one might already know by having digged a bit deeper into the most-widely adopted GraphQL clients).
...
But we are not forced to use WebSockets at all. HTTP Long-Polling (e.g. via [Server Side Events](https://developer.mozilla.org/en-US/docs/Web/API/Server-sent_events/Using_server-sent_events)) might be a more lightweigt solution. For both our server and client.
...
We are free to choose the best transport for our application requirements!
...
Usually we will have a network interface that is called by our favorite GraphQL client. Every single one has a different naming and approach of how they work. Apollo calls them links, relay calls the fetcher functions, and urql calls them exchanges.
...
A lot more data than necessary could be transported over the wire to the client. Even if nothing has changed from the last poll interval.

3. Flexibility. **Yes.** ✅

Straight forward, as this does not rely needs any changes on the server.
...
When using services as Postgraphile and Hasura that is the case. However, for any additional resolvers that are added on top of the service schema we cannot implement invalidation an invalidation mechanism.
...
Both Hasure and Postgraphile are tightly coupled to SQL database? For our own solutions we have to come up with resource tracking invalidations ourselves.
...
Adding a immutable layer to our existing schema might be a pretty big change. Furthermore, the library does lack support for some GraphQL features such as Interfaces and Unions.

## GraphQL Live Queries via the `@live` directive

There are companies out there, like [Facebook](https://www.youtube.com/watch?v=BSw05rJaCpA), that are already using this approach. There is also a graphql framework available in go that supports live queries out of the box! Check out [thunder](https://github.com/samsarahq/thunder).
...
When using a ORM such as prisma we can simply add a middleware for automatically invalidating resources.
...
In case our execution result payloads are getting to big we can easily enable JSON patches by adding a middleware.
...
- Partial query re-execution. (which actually already do-able today with relay conform schemas by building an ad-hoc query or by building an adhoc schema on the fly)
