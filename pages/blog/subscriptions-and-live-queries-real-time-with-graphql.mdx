...
const resolvers = {
  ...
  Mutation: {
    postAdd: (_, args, context) => {
      ...
      // We don't want to publish the whole object via our event emitter. The id should be sufficient.
...
const ChatRenderer = () => {
  ...
  useSubscription(OnPlaySound, useMemo({
    onNext: (payload) => {
...
Furthermore, the library is not maintained that well. I've made PRs to make the library compatible with recent GraphQL versions, but these have yet to be merged. Using unions and interfaces is not possible.```
...
I hope that more people will try exploring the possibilities of GraphQL live queries. Eventually, we could even come up with an official RFC for the spec!

There is a lot of more to explore such as
...
- A better format for patches (as JSON patch performs rather poorly on list diffs)
