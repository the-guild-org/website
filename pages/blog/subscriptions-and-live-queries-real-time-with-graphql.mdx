...
Straight forward, as this does not rely on any changes on the server.

4. Adaptability. **Yes.** ✅

Straight forward, again no changes on the server required!
...
All following values, will have no `query` value (`null`), but instead a array of patch operations that describe the changes for updating the last result to the next result.
...
- Partial query re-execution (which is actually already doable today with Relay conform schemas by building an ad-hoc query or by building an ad-hoc schema on the fly)
- Mechanisms for batching similar query operations (aka prevent executing the same operation multiple times when it is invalidated)
