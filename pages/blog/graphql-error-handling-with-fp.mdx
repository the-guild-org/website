...
```typescript
// unsafe API
const isUserAllowedForEntity = async (
  user: User,
  entity: Entity
): Promise<boolean> => {
  try {
...
export const isUserAllowedForEntityAsError =
  ...
  (entity: Entity): TE.TaskEither<NotAllowedError | UnknownError, boolean> =>
    pipe(
      ...
      TE.chainW(
        isNotAllowedAsError(
          `User ${user.id} isn't allowed to access entity ${entity.id}`
        )
      )
    )
```
...
export const getEntityForUser = (
  ...
  pipe(
    getUser(userId),
    TE.chainW((user) =>
      pipe(getEntity(id), TE.chainFirstW(isUserAllowedForEntityAsError(user)))
    )
  )
```
...
export const getEntityForUser = (
  ...
  pipe(
    ...
    TE.bind('isAllowed', ({ user, entity }) =>
      isUserAllowedForEntityAsError(user)(entity)
    ),
