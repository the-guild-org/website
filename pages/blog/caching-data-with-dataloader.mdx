...
The goal of caching is to improve performance where it’s needed the most.
...
For example, let's say we're trying to cache books. Our package name will be the books id, so we can tell which book is which.  
Our package data will be the books length, writer, price etc...
...
The reason we want to have types is for type saftey, let's say we have books and toys which have different values, we wouldn't want that somehow toys will be inserted into our books cache
...
The N+1 problem is a common problem when designing GraphQL API.
Looking at the Query below, we can see that the GraphQL API will call 20 times the `Book.author` resolver:
...
Depending on the resolver implementation, this query might trigger 20 SQL queries or API calls to resolve authors that might've wrote multiple books.
Dataloader helps to solve this problem by caching, deferring and grouping similar resolver calls.
