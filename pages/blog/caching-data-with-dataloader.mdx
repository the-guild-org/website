...
export const meta = {
  ...
  author: 'giladtidhar',
  tags: ['cache', 'data', 'servers', 'graphql'],
  date: '2022-1-12',
...
- Data that is queried a lot of times will be fetched from the database only one time, so there's less time lost for fetching multiple times.
...
### 2. Create a dataloader instant.

To start, import dataloader and LRU (LRU is a way to map cache data, it is installed with dataloader).
